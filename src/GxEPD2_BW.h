...
class GxEPD2_BW : public GxEPD2_GFX_BASE_CLASS
{
  public:
    ...
    void drawPixel(int16_t x, int16_t y, uint16_t color)
    {
      ...
      if( color == GxEPD_INVERT ){
        if(_buffer[i] & (1 << (7 - x % 8)))
          _buffer[i] = (_buffer[i] & (0xFF ^ (1 << (7 - x % 8))));
        else
          _buffer[i] = (_buffer[i] | (1 << (7 - x % 8)));
      }else if( color == GxEPD_GREY ){
        if( (x+y) % 2 ) 
          _buffer[i] = (_buffer[i] | (1 << (7 - x % 8)));
        else
          _buffer[i] = (_buffer[i] & (0xFF ^ (1 << (7 - x % 8))));
    ...
    */
    /**************************************************************************/
    void /*Adafruit_GFX::*/drawBitmapPart(int16_t x, int16_t y, const uint8_t bitmap[],
                                  int16_t w, int16_t h, int16_t x1, int16_t w1, uint16_t color,
                                  uint16_t bg) {
