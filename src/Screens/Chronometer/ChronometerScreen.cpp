#include "ChronometerScreen.h"

#include <time.h>

#include "northrup_serif8pt7b.h"
#include "WatchfaceBitmaps.h"
...
void ChronometerScreen::drawDate(String dayOfWeek, String date, int hourAngle,
                          int minuteAngle) {
...
void ChronometerScreen::drawWatchface(tm t, float battery) {
...
void ChronometerScreen::show() {
  // show once every minute
  //TODO AZ: Watchy_Event::setUpdateInterval(SECS_PER_MIN*1000);
...
void ChronometerScreen::demoMode() {
  for (int h = 0; h < 12; h++) {
    for (int m = 0; m < 60; m++) {
      tm t = {.tm_sec = 0,
              .tm_min = m,
