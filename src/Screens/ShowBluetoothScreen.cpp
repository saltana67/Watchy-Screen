...
class SSIDChanged : public BLECharacteristicCallbacks {
  public:
    ...
    */
    esp_err_t ret = setSSID(pCharacteristic->getValue().c_str());
    /*
  ...
  private:
   esp_err_t setSSID(const char * ssid) {
     log_i("set ssid %s", ssid);
     return ESP_OK;
     /*
     wifi_config_t conf;
     esp_err_t ret = esp_wifi_get_config(ESP_IF_WIFI_STA, &conf);
...
class PasswordChanged : public BLECharacteristicCallbacks {
  public:
   void onWrite(BLECharacteristic *pCharacteristic) override {
     ...
     */
    esp_err_t ret = setPassword(pCharacteristic->getValue().c_str());
     if (ret != ESP_OK) {
       log_e("error: %s", esp_err_to_name(ret));
     }
   };
  private:
   esp_err_t setPassword(const char * password){
     /*
     wifi_config_t conf;
     esp_err_t ret = esp_wifi_get_config(ESP_IF_WIFI_STA, &conf);
     if (ret == ESP_ERR_WIFI_NOT_INIT) {
...
//      BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE
  ...
  */
  //const char *pwd = "watchypwd";
  uint8_t ssid[32] = "watchyssid";
  uint8_t password[64] = "watchypwd";

  pCharacteristic->setValue(
      //conf.sta.ssid,
      ssid,
      //strnlen(reinterpret_cast<char *>(conf.sta.ssid), sizeof(conf.sta.ssid)))
      strnlen(reinterpret_cast<char *>(ssid), sizeof(ssid))
  );
...
//      BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE
  ...
  pCharacteristic->setValue(
      ...
      strnlen(reinterpret_cast<char *>(password), sizeof(password))
  );
  pService->start();
